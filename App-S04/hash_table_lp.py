...
def hash_table(capacity, loadfactor, cmpfunction=None):
...
def find_key_ignorar(ht, key):
    ...
    if ht['keys'][pos] == None:
        return None
    elif ht['keys'][pos]!= None: #esto es re extra pero si no lo pongo falla:(
        if ht['keys'][pos][0] == key:
            return pos
        else:
...
def value_Set(ht):
    lista_values = nf.newList()
    for pos in ht['keys'].values():
        nf.addLast(lista_values, pos)
    return lista_values
