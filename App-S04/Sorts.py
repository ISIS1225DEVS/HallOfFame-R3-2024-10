...
def published_salario(a, b):
    if a["published_at"] < b["published_at"]:
        return True
    elif a["published_at"] == b["published_at"]:
        if a['salary'] == "Unknown" and b['salary'] != "Unknown":
            return True
        elif b['salary'] == "Unknown" and a['salary'] != "Unknown":
            return False
...
diccionario = {"elements": [{"id": 4}, {"id": 2}, {"id": 3}, {"id": 0}], "size": 4, "type": "ARRAY_LIST"}
chain = nf.newList("SINGLE_LINKED")
for i in diccionario["elements"]:
    nf.addLast(chain, i)
    
#print(TimSort(diccionario,mayor_menor_p))